...
PL = set(PL,'name','force','type','p',...
    'parameters',[25 5 -1],'range',rng,'label','positive large');
PL = calcVVector(PL,0);
PM = fuzzyVar();
PM = set(PM,'name','force','type','p',...
    'parameters',[13 5 6],'range',rng,'label','positive medium');
PM = calcVVector(PM,0);
PS = fuzzyVar();
PS = set(PS,'name','force','type','p',...
    'parameters',[7 4 5],'range',rng,'label','positive small');
PS = calcVVector(PS,0);
PVS = fuzzyVar();
...
NM = set(NM,'name','force','type','p',...
    'parameters',[-9 6 5],'range',rng,'label','negative medium');
NM = calcVVector(NM,0);
NS = fuzzyVar();
...
ZE = set(ZE,'name','force','type','p',...
    'parameters',[-2 1 1],'range',rng,'label','zero');
ZE = calcVVector(ZE,0);
